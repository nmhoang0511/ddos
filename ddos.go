...
func main() {
	...
	go func() {
		fmt.Println("-- Tool ÄĂ£ Báº¯t Äáº§u Khá»Ÿi Äá»™ng --\n           Go!\n\n")
		ss := make(chan uint8, 8)
		var (
			err, sent int32
		)
		fmt.Println("Sá»‘ Byte Gá»­i Ä‘i               |\tGá»­i ThĂ nh CĂ´ng |\tGá»­i KhĂ´ng ThĂ nh CĂ´ng")
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- Tool ÄĂ£ Äáº¡t Váº¡ch ÄĂ­ch --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
